class Gfg {
    ...
    // It can be fresh but might have rotten true since the matrix isn't actually
    // updated, only rotten keeps track
    static boolean canRot(int[][] a, int row, int col, boolean[][] rotten) {
        return (row >= 0) && (row < a.length) && (col >= 0) && (col < a[0].length) && (a[row][col] == 1)
                && (rotten[row][col] == false);
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t-- > 0) {
            ...
            // Queue
            //Collections can use primitive arrays (Primitive arrays are objects)
            //int[] will work here
            ...
            while (!queue.isEmpty()) {
                //Can use size here instead of time delimiter
